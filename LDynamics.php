    ...
    */
    ...
    private function performRequest($endpoint, $method, $payload = false, $customHeaders, $originMethod) {
        try {
            ...
            if (!$token["success"]) {
                return new LDynamicsResponse(json_encode(array(
                    'error'     => array(
            ...
            return new LDynamicsResponse($responseBody, $responseHeaders, $endpoint, $originMethod, $rawResponse);
        } catch (\Exception $e) {
            return false;
        }
    }
...
class LDynamics {
    ...
    public function performBatchRequest($payload, $batchID) {
        $worker = new LDynamicsWorker('contacts', $this->config);
        return $worker->performBatchRequest($payload, $batchID);
    }

    public function __get($prop) {
        $worker = new LDynamicsWorker($prop, $this->config);
        return $worker;
    }
}
